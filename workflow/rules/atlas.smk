...
mrt_root = os.path.join(config["root"], "{dataset}","mrtrix")
...
rule merge_lr_dseg:
    input:
        left=bids(root=mrt_root,datatype='dwi',hemi='L',atlas='{atlas}',suffix='dseg.nii.gz',
                **config['subj_wildcards']),
        right=bids(root=mrt_root,datatype='dwi',hemi='R',atlas='{atlas}',suffix='dseg.nii.gz',
                **config['subj_wildcards'])
    output:
        merged=bids(root=mrt_root,datatype='dwi',atlas='{atlas}',suffix='dseg.nii.gz',
                **config['subj_wildcards'])
    shell: 
